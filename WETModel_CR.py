...
def changetemp(y,x,z,f1, T, Tnew, ip):
    
    if x == 13 and y == 13 and z < 20:
        Tnew[y,x,0] = (ip/(sigma*0.8*20*4*dx*dy))**0.25
        Tnew[y,x,z] = (ip/(sigma*0.8*20*4*dx*dy))**0.25
        
        
    pmw = ip/(20*dx*dy*4)    
    if y == 13 and x == 12 and z < 20:    
        deltaP = pmw*dx*dy*dt/(c[y,x,z]*rho[y,x,z]*dx*dy*dz)+ f1[y,x,z]*(T[y+1,x,z]+T[y,x+1,z]+T[y,x,z+1]+T[y-1,x,z]+T[y,x-1,z]+T[y,x,z-1]-6*T[y,x,z])+T[y,x,z]
        Tnew[y,x,z] = T[y,x,z] + deltaP

    elif y == 13 and x == 14 and z < 20:      
        deltaP = pmw*dx*dy*dt/(c[y,x,z]*rho[y,x,z]*dx*dy*dz)+ f1[y,x,z]*(T[y+1,x,z]+T[y,x+1,z]+T[y,x,z+1]+T[y-1,x,z]+T[y,x-1,z]+T[y,x,z-1]-6*T[y,x,z])+T[y,x,z]
        Tnew[y,x,z] = T[y,x,z] + deltaP
        
    elif y == 12 and x == 13 and z < 20:         
        deltaP = pmw*dx*dy*dt/(c[y,x,z]*rho[y,x,z]*dx*dy*dz)+ f1[y,x,z]*(T[y+1,x,z]+T[y,x+1,z]+T[y,x,z+1]+T[y-1,x,z]+T[y,x-1,z]+T[y,x,z-1]-6*T[y,x,z])+T[y,x,z]
        Tnew[y,x,z] = T[y,x,z] + deltaP
        
    elif y == 14 and x == 13 and z < 20:       
        deltaP = pmw*dx*dy*dt/(c[y,x,z]*rho[y,x,z]*dx*dy*dz)+ f1[y,x,z]*(T[y+1,x,z]+T[y,x+1,z]+T[y,x,z+1]+T[y-1,x,z]+T[y,x-1,z]+T[y,x,z-1]-6*T[y,x,z])+T[y,x,z]
        Tnew[y,x,z] = T[y,x,z] + deltaP
        
    elif y == 13 and x == 13 and z ==20:   
        deltaP = pmw*dx*dy*dt/(c[y,x,z]*rho[y,x,z]*dx*dy*dz)+ f1[y,x,z]*(T[y+1,x,z]+T[y,x+1,z]+T[y,x,z+1]+T[y-1,x,z]+T[y,x-1,z]+T[y,x,z-1]-6*T[y,x,z])+T[y,x,z]
        Tnew[y,x,z] = T[y,x,z] + deltaP
...
def calculate(ip, time, T, p1, SEED):
    ...
    for t in t_array:
        ...
        if t%100 == 0:
            print('SEED: ' +str(SEED)+', Input Power: ' +str(ip)+'W, Water Conc.: ' +str(round(wcountinitial*100,1))+'%, Total Time: ' +str(time)+'s, Running.... '+str(t)+'s')
        ...
        for y in range(1, yend):
            for x in range(1, xend):
                for z in range(1, zend):
                    ...
                    changetemp(y,x,z,f1, T, Tnew, ip)     
    
        
        T = Tnew.copy()


ip = 450 #Input Power in W, calculated from repsective input power
...
calculate(ip, TIME, T,p1, SEED) #simulates the particular heating run
