...
def BoundCondition(y,x,z,T):
    if z == 0 or z == zend:
       if z==0:
          ...
          Tnew[y,x,0] = ((cf*Q*(1-A0))/(sigma*e))**0.25
...
def calculate(cf, time, T, p1, SEED):
    ...
    for t in t_array:
        ...
        if t%100 == 0:
            print('SEED: ' +str(SEED)+', Concentration Factor: ' +str(cf)+', Water Conc.: ' +str(round(wcountinitial*100,1))+'%, Total Time: ' +str(time)+'s, Running.... '+str(t)+'s')
...
cf = 100 #Concentration Factor, calculated from repsective input power
...
calculate(cf, TIME, T,p1, SEED) #simulates the particular heating run
